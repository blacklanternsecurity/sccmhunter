...
class AddComputerSAMR:
    ...
    def __init__(self, username, password, domain, hashes, aes, kerberos, dc_ip, computer_name=None,computer_pass=None):
        ...
        try:
            ...
            if self.__noAdd or self.__delete:
                ...
                try:
                    ...
                    if e.error_code == 0xc0000022:
                        print("[-] User %s doesn't have right to create a machine account!" % self.__username)
                        sys.exit()
                    elif e.error_code == 0xc00002e7:
                        print("[-] User %s machine quota exceeded!" % self.__username)
                        sys.exit()
                    else:
                        raise
